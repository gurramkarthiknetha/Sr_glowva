export default function TrendingProducts() {
	...
	return(
		<div className="max-w-7xl mx-auto px-4 py-8 bg-gradient-to-tr from-[#fff5e1] via-[#f7cac9] to-[#355c7d] rounded-3xl border-4 border-dashed border-[#f7786b] shadow-[0_8px_32px_0_rgba(247,202,201,0.25)] relative overflow-hidden">
			<h1 className="text-4xl font-extrabold mb-6 text-center text-transparent bg-clip-text bg-gradient-to-r from-[#f7786b] via-[#355c7d] to-[#b5ead7] drop-shadow-lg tracking-widest mix-blend-multiply animate-pulse">Trending Products</h1>
			<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
				{products.slice(0, 8).map((product) => (
					<div
						key={product.id}
						className="bg-white/90 rounded-2xl shadow-xl border-2 border-double border-[#b5ead7] relative flex flex-col hover:shadow-2xl transition-shadow duration-200 hover:scale-105 hover:border-[#f7786b] hover:bg-[#ffdac1]/80 backdrop-blur-md"
					>
						<div className="relative">
							<img
								src={product.thumbnail}
								alt={product.title}
								className="w-full h-56 object-cover rounded-t-2xl border-b-4 border-dotted border-[#f7cac9]"
							/>
							<span className="absolute top-3 right-3 bg-gradient-to-r from-[#f7786b] to-[#b5ead7] text-white text-xs font-bold px-3 py-1 rounded-full z-10 shadow-lg border-2 border-white animate-bounce">
								{product.discountPercentage}% off
							</span>
							<button className="absolute bottom-3 right-3 bg-white/80 rounded-full p-2 border-2 border-[#355c7d] hover:bg-[#b5ead7]/60 transition-colors" aria-label="Wishlist">
								<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 text-[#f7786b]">
						...
						<div className="flex-1 flex flex-col justify-between p-4">
							<div>
								<h2 className="text-lg font-bold mb-1 text-[#355c7d] drop-shadow-sm font-serif">{product.title}</h2>
								<div className="flex items-center gap-2 mb-2">
									<span className="text-[#f7786b] font-extrabold text-xl">₹{Math.round(product.price)}</span>
									<span className="text-gray-400 line-through text-base">₹{Math.round(product.originalPrice)}</span>
								</div>
							</div>
							<div className="flex items-center gap-2 mt-4">
								<button className="border-2 border-[#f7cac9] rounded-full p-2 text-[#f7786b] hover:bg-[#f7cac9]/40 transition-colors" aria-label="WhatsApp">
								...
								<button className="flex-1 border-2 border-[#355c7d] text-[#355c7d] font-bold rounded-full py-2 transition-colors hover:bg-[#b5ead7]/40 text-base font-mono">
									{product.buttonText}
								</button>
							</div>
						</div>
